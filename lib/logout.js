...
module.exports.logout = function (options) {
  ...
  function prepareAndSendLogoutRequest(sessions, transactionId, req, res, next) {
    // Finished if there are no more session - finish logout
    if (!sessions || sessions.length === 0) { return finalize(transactionId, req, res, next); }
    ...
    options.store.load(req, transactionId, {destroy: true}, function (err, transaction) {
      ...
      options.clearIdPSession(function (err) {
        ...
        var logoutResponse = templates.logoutresponse({
          ...
          destination:    data.serviceProviderLogoutURL, // Destination taken from session (matches issuer from the LogoutRequest with session serviceProviderId)
      ...
      options.sessionParticipants.getAll(function (err, sessions) {
        ...
        if (req.query.SAMLRequest || req.body.SAMLRequest) {
          ...
          parseIncomingLogoutRequest(req, req.query.SAMLRequest || req.body.SAMLRequest, opts, function (err, requestData) {
            ...
            var spData = {
              parsedRequest: {
                id: requestData.id,
                serviceProviderLogoutURL: session.serviceProviderLogoutURL
              },
            ...
            options.store.save(req, spData, function (err, transactionId) {
              ...
              options.sessionParticipants.remove(requestData.issuer, function (err, updatedSessions) {
                if(err) return next(err);
                
                sessions = updatedSessions;
                prepareAndSendLogoutRequest(sessions, transactionId, req, res, next);
              });
            });
          });
        ...
        // Logout flow in progress, incoming SAMLResponse from SP. (Could be SP initiated or IdP initiated)
          ...
          function processLogoutResponse(logoutResponse, transactionId) {
            ...
            options.sessionParticipants.remove(logoutResponse.issuer, function (err, updatedSessions) {
              if(err) return next(err);

              // Continue with next session if any
              prepareAndSendLogoutRequest(updatedSessions, transactionId, req, res, next);
            });
          }
