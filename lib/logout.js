    ...
    options.sessionHandler.clearIdPSession(function (err) {
      ...
      options.store.get(function (err, data) {
        ...
        var logoutResponse = templates.logoutresponse({
          ...
          destination:    data.destination || options.destination, // Destination taken from session (match issuer from the LogoutRequest with session serviceProviderId)
        ...
        options.destination = data.destination || options.destination;
  ...
  return function (req, res, next) {
    try {
      ...
      if (req.query.SAMLRequest || req.body.SAMLRequest) {
        parseIncomingLogoutRequest(req, req.query.SAMLRequest || req.body.SAMLRequest, { signingCert: options.cert, thumprints: options.thumprints }, function (err, requestData) {
          if (err) { return next(err); }
          ...
          options.sessionHandler.getActiveSessions(function (err, sessions) {
            ...
            requestData.destination = session.serviceProviderLogoutURL;
...
function parseIncomingLogoutRequest(req, samlRequest, options, callback) {
  utils.parseSamlRequest(req, samlRequest,  "LOGOUT_REQUEST", options, function (err, logoutRequestDom) {
