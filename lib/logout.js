...
module.exports.logout = function (options) {
  ...
  options.store = options.store || (options.sloEnabled ? new SessionStore(options) : new InMemoryStore());
  ...
  function finalize (req, res, next) {
    options.clearIdPSession(function (err) {
      ...
      if (err) options.store.setLogoutStatusFailed(req);

      options.store.getSpInitiated(req, function (err, data) {
        if (err) return next(err);
        ...
        var logoutResponse = templates.logoutresponse({
          ...
          samlStatusCode: options.store.isLogoutFailed(req) ? STATUS.PARTIAL_LOGOUT : STATUS.SUCCESS
        });
        ...
        options.relayState = data.relayState;
      ...
      options.sessionParticipants.getAll(function (err, sessions) {
        ...
        if (req.query.SAMLRequest || req.body.SAMLRequest) {
          ...
          parseIncomingLogoutRequest(req, req.query.SAMLRequest || req.body.SAMLRequest, opts, function (err, requestData) {
            ...
            options.store.storeSpInitiated(req, requestData, function (err) {
              if (err) { return next(err); }
          ...
          options.store.verifyState(req, req.query.RelayState || (req.body && req.body.RelayState), function(err, ok, state){
            ...
            validateSamlResponse(req, sessions, function (err, logoutResponse) {
              ...
              options.sessionParticipants.remove(logoutResponse.issuer, function(err){
                ...
                // LogoutResponse was OK, we remove the session participant from our in memory list
...
function removeSession(sessions, logoutResponse) {
  ...
  var sessionIndexToRemove = sessions.findIndex(function (session) {
    return logoutResponse && session.serviceProviderId === logoutResponse.issuer;
  });
