...
module.exports.validateXmlSignature = validateXmlSignature;

function validateXmlSignature(xml, options, callback) {
  var calculatedThumbprint = '';
  var signature = options.signature ? xpath.select(options.signature, xml)[0] : xpath.select(signaturePath, xml)[0];
  if (!signature){
    return callback(new Error('Signature is missing (xpath: ' + (options.signature || signaturePath) + ')'));
  }
  ...
  var sig = new SignedXml(null, { idAttribute: 'ID' });
